...
percentileDemo <- function(x, labels.signif=3, pctile.color='RoyalBlue', mean.color='Orange', range.color='DarkRed', boxp=FALSE, ...) {
  ...
  .summary <- function(x, pr=c(0.05, 0.1, 0.5, 0.9, 0.95)) {
    ...
    d <- data.frame(mean=x.mean, sd=x.sd, min=x.min, max=x.max, q, n)
    return(d)
  }
  ...
  # setup plot
  plot(d.x, type='n', ylim=c(-0.25, 1.1), axes=FALSE, ...)
  abline(h=c(0, 1), col=grey(0.85))
  lines(d.x, lwd=2)
  lines(ideal.normal)
  
  # add obs as vertical ticks
  # points(x, rep(1, times=length(x)), pch='|', cex=0.5, col=grey(0.7))
...
# Introduction
This document describes the *percentile* as a robust measure of "place" within a distribution of values. Examples are given for its application in the context of soil data summaries.
...
The following figures demonstrate the relationship between distribution shape, measures of central tendency (mean and 50th percentile, and measures of spread (mean +/1 2 standard deviations, and 10th / 90th percentiles). Within each figure is an *idealized* normal distribution that is based on the sample mean and standard deviation.
...
Various forms of the [log-normal distribution](https://en.wikipedia.org/wiki/Log-normal_distribution) are typically more accurate approximations of soil properties. Log-normal distributions with a "short tail", or a low degree of [asymmetry](https://en.wikipedia.org/wiki/Skewness) around the central tendency, are common. Note the shift between mean and median, and the unequal distances to 10th and 90th percentiles. Examples include lab measured organic carbon and field measured rock fragment volume.
```{r echo=FALSE, warning=FALSE, fig.width=9, fig.height=5}
par(mar=c(3,1,2,1))
set.seed(101010)
...
Log-normal distributions with a "long tail", or a high degree of [asymmetry](https://en.wikipedia.org/wiki/Skewness) around the central tendency, are commonly encountered when summarizing GIS data sources such as elevtion, slope, and curvature. Note that the mean +/- 2SD is now longer a meaningful representation of spread.
```{r echo=FALSE, warning=FALSE, fig.width=9, fig.height=5}
par(mar=c(3,1,2,1))
set.seed(101010)
...
Consider some data from the KSSL characterization database. 

```{r echo=FALSE, warning=FALSE, fig.width=9, fig.height=5}
par(mar=c(4.5,1,2,1))
x <- subsetData(kssl, hzname = 'hzn_desgn', hz_pat = 'B', var = 'clay')
res <- percentileDemo(x, xlab='Percent Clay', main='KSSL: Miami, B Horizons')
kable(res, digits = 1)
```
...
res <- percentileDemo(x, xlab='pH (%)', main='KSSL: Miami, B Horizons')
kable(res, digits = 1)
```
...
res <- percentileDemo(x, xlab='Total Fragment Volume', main='NASIS: Loafercreek, B Horizons')

x <- subsetData(loafercreek, hzname = 'hzname', hz_pat = 'B', var = 'clay')
res <- percentileDemo(x, xlab='Percent Clay', main='NASIS: Loafercreek, B Horizons')
```
...
```{r  echo=FALSE, results='hide', warning=FALSE, fig.width=9, fig.height=6.5}
par(mar=c(4.5,1,3,1))
res <- percentileDemo(e.elev, xlab='Elevation (m)', main='CA630: 5012')
res <- percentileDemo(e.slope, xlab='Slope (%)', main='CA630: 5012')
res <- percentileDemo(e.MAP, xlab='PRISM Mean Annual PPT (mm)', main='CA630: 5012')
```
